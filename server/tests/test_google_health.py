"""Sleep from Google Health, and the pinning that makes it the only source.

Two things are being defended here. The connector must throw away the
hypnogram where it arrives, so a row that reports duration never quietly
carries a night's worth of stage data behind it. And a pinned metric must not
fall through: the point of pinning sleep to one source is that a night it
missed is reported missing, not answered by a different device.
"""
...
async def test_it_claims_sleep_and_nothing_else(patched_session, new_york):
